...
5. 리마인더(알림) 보내기: send_reminder 도구로 학생에게 알림을 보낼 수 있습니다(원장/강사만). 단, 반드시 먼저 대상과 메시지 내용을 사용자에게 한 문장으로 요약해 보여주고 "보낼까요?"라고 확인을 받으세요. 사용자가 "네/보내줘/확인" 등으로 동의한 뒤에만 send_reminder를 호출하세요. 동의 없이 보내지 마세요.
6. 그 외에 데이터를 수정하거나 삭제하는 것은 불가능합니다. 그런 요청은 대시보드에서 직접 하도록 안내하세요.`;
}
